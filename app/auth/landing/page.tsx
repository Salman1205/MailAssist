...
/* Branded frame matching the homepage. It follows the GLOBAL theme (light or
   dark via the html.dark class) so the whole app stays consistent, and it
   collapses each form's built-in two-column marketing layout into one clean,
   centered card. */
...
const AUTH_CSS = `
.auth-frame{
  --paper:#F6F5F1; --paper-2:#FFFFFF; --ink:#15181C; --muted:#5B6169; --teal:#0C8B99; --line:#E4E1D9; --line-2:#D8D4CA;
  ...
  color:var(--ink); font-family:var(--sans); letter-spacing:-.011em; display:flex; flex-direction:column;
}
/* Follow the global theme: dark when the app is in dark mode. */
.dark .auth-frame{
  --paper:#0E1216; --paper-2:#161B22; --ink:#EAEDF1; --muted:#98A0AA; --line:rgba(255,255,255,.1); --line-2:rgba(255,255,255,.16);
...
.auth-center{position:relative; z-index:2; flex:1; display:flex; align-items:center; justify-content:center; padding:8px 20px 56px;}
...
.auth-choice-ic{width:42px; height:42px; border-radius:11px; display:grid; place-items:center; background:rgba(12,139,153,.12); color:var(--teal); margin-bottom:6px;}
